...
class TaysirSupabaseUser extends BaseAuthUser {
  TaysirSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> taysirSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TaysirSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
